...
def plot_plate():
    dfi = df[df['currency'] == 'EUR']
    dfi['Traded_USD'] = dfi['Traded_USD']* 10000
    dfi = dfi[dfi['exchange'].isin(['EURONEXT','XETR'])]
    ...
    #                  path=['plate','','sector','industry',],
                    ...
                    color='change', 
                    custom_data=['change','name','market_cap_USD','close','市场','Traded_USD','exchange'],
    ...
    figi.update_traces(textposition='middle center', 
                    ...
                    hovertemplate= "%{customdata[1]}<br>%{label}<br>%{customdata[3]:.3f} (%{customdata[0]:.2f})%<br>%{customdata[4]} %{customdata[6]}<br>总市值=%{customdata[2]:.1f}亿<br>成交额=%{customdata[5]:d}万"                  
