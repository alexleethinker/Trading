...
translated_name = './data/static/tw_all_securaties.csv'
...
tw_df = pd.read_csv(translated_name).fillna('')

df = df.merge(trans_df, on = 'industry').merge(market_df, on = 'market').merge(tw_df, how= 'left', left_on = 'name', right_on='有价证券代号')
...
def plot_plate(market):
    ...
    figi = px.treemap(dfi, 
                    path=[px.Constant("台股"),'大行业','一级行业','二级行业','有价证券名称'],  # 指定层次结构，每一个层次都应该是category型的变量
...
# Plot!
st.plotly_chart(plot_plate('taiwan'), use_container_width=True)
