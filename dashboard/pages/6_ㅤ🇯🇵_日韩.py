...
def plot_plate():
    ...
    figi = px.treemap(dfi, 
                    path=[px.Constant("亚太"),'市场','一级行业','二级行业','description'],  # 指定层次结构，每一个层次都应该是category型的变量
    ...
    figi.update_traces(textposition='middle center', 
                    ...
                    hovertemplate= "%{label}<br>%{customdata[1]}<br>%{customdata[3]:.1f} (%{customdata[0]:.2f})%<br>总市值=%{customdata[2]:d}亿"                  
