...
def stock_spot():
    ...
    temp_df['股票代码'] = temp_df['代码'].apply(market_suffix)
    temp_df['股票简称'] = temp_df['名称'].str.replace(' ','').str.replace('Ａ','A')
    ...
    temp_df = temp_df[['股票代码','股票简称','流通市值','总市值','最新价', '涨跌幅']]

    stock_custom_industry = pd.read_csv('./csv/stock_custom_industry.csv',encoding = 'gbk').drop(['股票简称'], axis=1)
    df = temp_df.merge(stock_custom_industry,how='left',on=['股票代码'])
...
def update_spot_data():   
    try:
        spot = stock_spot()
        spot.to_csv('./csv/stock_spot.csv', index = False, encoding = 'utf-8')
    except:
        pass
