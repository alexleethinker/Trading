...
class StockSpotEuronext():
    ...
    def clean(self, euronext_df):
        global_df = pd.read_csv( data_dir + '/spot/stock_spot_global_all.csv').drop(columns=['change','currency'])
        # df = euronext_df.merge(global_df, how = 'left', left_on = ['symbol','market'], right_on = ['name','market'])
        degiro_df = pd.read_csv( data_dir + '/static/euronext_degiro.csv') 
    ...
    def update(self, df):
        df.to_csv( data_dir + '/spot/stock_spot_euronext.csv', index = False, encoding = 'utf-8')
